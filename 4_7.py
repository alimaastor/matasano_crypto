...
def is_previous_guess_wrong(timeouts, signature, server_timeout):
    ...
    if avg_time < minimum_timeout:
        print 'avg_time greater than server_tiumeout'
        return True
    else:
        return False
    ...
    try:
        ...
        while True:
            ...
            if is_previous_guess_wrong(timeouts, signature, server_timeout):
                ...
                print 'best candidate is', get_best_candidate(timeouts)
                for i in timeouts.iteritems():
                    print i
                print '*'*50
...
def main():
    # Init variables.
    os.environ['SERVER_TIMEOUT'] = "0.0050"
    ...
    assert len(signature) == SIGNATURE_LEN, "Signature not found! {} != {}".format(len(signature), SIGNATURE_LEN)
    print 'signature is', signature
